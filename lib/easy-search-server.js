    ...
    Searchers = {
        'elastic-search': {
            ...
            'getMongoDocumentObject': function(data) {
                ...
                var mongoDbDocFake = [];
                _.each(data.hits.hits, function(resultSet) {
                    mongoDbDocFake.push(resultSet["_id"]);
                });
                return mongoDbDocFake;
            },
            /**
            ...
            'search': function(name, searchString, fields, callback) {
                ...
                if (index.onSearch && "function" === typeof index.onSearch) {
                    if (!index.onSearch(name, searchString, fields, queryObj)) {
                        return;
                    }
                }

                var queryObjCount = _.clone(queryObj);
                delete queryObjCount.size;
                delete queryObjCount.sort;

                queryObj.fields = [];
                // Most likely client call, return data set
                ElasticSearchClient.count(name, 'default_type', queryObjCount, function(error, dataCount) {
                    var resData = {};
                    resData.count = JSON.parse(dataCount);
                    resData.count = resData.count.count;

                    ElasticSearchClient.search(name, 'default_type', queryObj, function(error, data) {
                        resData.result = String(that.getMongoDocumentObject(data));
                        fut['return'](resData);
        ...
        'mongo-db': {
            ...
            'getSelector': function(index, searchString) {
                var orSelector,
                    selector = {},
                    field = index.field,
                    stringSelector = index.exact ? searchString : {
                        '$regex': '.*' + searchString + '.*'
                    };
            ...
            'search': function(name, searchString) {
                ...
                return index.collection.find(selector, {
                    'limit': index.limit
                }).fetch();
            }
        }
